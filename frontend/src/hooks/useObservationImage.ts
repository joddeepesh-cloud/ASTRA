...
export function useObservationImage(obs: Observation | null | undefined): { imageUrl: string; isFallback: boolean; isLoading: boolean } {
  ...
  useEffect(() => {
    ...
    async function resolveImage() {
      ...
      // 1. Stable library observation asset (/assets/ or http:// or https://)
      if (
        obs.image_url &&
        !obs.image_url.includes('USER FILE') &&
        !obs.image_url.startsWith('blob:') &&
        !obs.image_url.startsWith('data:') &&
        (obs.image_url.startsWith('http') || obs.image_url.startsWith('/assets/'))
      ) {
        if (isMounted) {
      ...
      if (match?.image_key) {
        try {
          ...
          if (histBlob && isMounted) {
            ...
            setIsLoading(false);
            if (isDev) {
              console.log(`[ImageResolver] History match SUCCESS for ${obs.id} (Record: ${match.id}, Key: '${match.image_key}', Blob size: ${histBlob.size} bytes)`);
            }
            return;
          }
        } catch (err) {
