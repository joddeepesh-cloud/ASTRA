...
MANIFEST_PATH = "ml/data/domain_gate_v2/manifest.csv"
SPLITS_PATH = "ml/data/splits/domain_gate_v2_splits.csv"

BEST_MODEL_PATH = "ml/models/domain_gate_v2_best.pt"
LATEST_MODEL_PATH = "ml/models/domain_gate_v2_latest.pt"
HISTORY_PATH = "ml/artifacts/domain_gate_v2_history.json"
...
def train_model():
    ...
    print(f"Using compute device: '{device}' for Domain Gate V2 training.")
    ...
    train_dataset = DomainGateDatasetV2(df_train, transform=train_tf)
    val_dataset = DomainGateDatasetV2(df_val, transform=eval_tf)
    ...
    print("\nStarting Domain Gate V2 Training Pipeline (Phase 1: Frozen Backbone, Phase 2: Fine-Tuning)...")
    
    for epoch in range(1, total_epochs + 1):
        if epoch == epochs_phase1 + 1:
            print("\n--- Transitioning to Phase 2: Unfreezing Backbone for Full Fine-Tuning ---")
        ...
        epoch_ms = (time.time() - start_t) * 1000.0
        
        val_score = val_metrics['f1'] * 0.5 + val_metrics['roc_auc'] * 0.5
        ...
        print(f"Epoch {epoch:02d}/{total_epochs:02d} ({epoch_ms:.0f}ms) | "
              ...
              f"F1: {val_metrics['f1']:.4f}, ROC-AUC: {val_metrics['roc_auc']:.4f}", flush=True)
        ...
        checkpoint_meta = {
            ...
            'val_metrics': val_metrics,
            'val_loss': val_loss,
            'model_version': f"mobilenet_v3_small_domain_gate_v2_epoch{epoch}"
        }
        ...
        if val_score >= best_val_score:
            ...
            print(f"  [SAVED] New best Domain Gate V2 checkpoint -> {BEST_MODEL_PATH}", flush=True)
            
    with open(HISTORY_PATH, "w") as f:
        json.dump(history, f, indent=2)
        
    print(f"\nDomain Gate V2 Training Complete! Best Validation Score: {best_val_score:.4f}")
